...
\title{Replication of Waggoner (1997) Spline Methods for Extracting Interest Rate Curves from Bond Prices}
\author{Annie Reynolds \& Phoebe Fingold}
...
\caption{Comparison of replication results to the original paper for the in-sample period. The table reports weighted mean absolute pricing error (WMAE) and bid-ask hit rates across maturity buckets for the McCulloch, Fisher, and Waggoner spline methods. The key takeaway is that the replicated models broadly reproduce the performance patterns reported in the original study, confirming that the implementation successfully matches the behavior of the published methods.}
\label{tab:1a}
\end{table}
...
\caption{Out-of-sample performance of the replicated spline models compared with the original paper. The table evaluates WMAE and bid-ask hit rates across maturity buckets. The main takeaway is that the replication produces similar relative performance across methods and maturities, suggesting the implementation generalizes reasonably well outside the estimation sample.}
\label{tab:1b}
\end{table}
...
\caption{Model fitting performance using the modern Treasury sample (last 20 years). The table reports in-sample WMAE and hit rates for each spline method across maturity buckets using more recent Treasury data. The key takeaway is that the relative ranking of methods remains similar to the historical replication, indicating that the spline approaches behave consistently even when applied to a much denser modern bond market.}
\label{tab:2a}
\end{table}
...
\caption{Out-of-sample performance of spline models using the modern Treasury sample (past 20 years). The table shows pricing errors and hit rates across maturity buckets. The main takeaway is that model fit generally deteriorates slightly at longer maturities and out-of-sample, highlighting where spline methods struggle most when predicting bond prices.}
\label{tab:2b}
\end{table}
...
Two such charts are shown and analyzed below. (Figures 1 and 2)

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../_output/methods_vs_gsw_low_corr_discount.png}
\caption{Comparison of discount curves from the spline methods and the GSW benchmark on the date with the lowest correlation between approaches. The figure highlights where spline methods diverge most from the parametric benchmark, illustrating how methodological differences can produce noticeably different curve shapes, particularly at longer maturities.}
...
\caption{Comparison of instantaneous forward curves produced by the spline-based methods and the GSW benchmark. While the overall shape of the forward curve is broadly consistent across models, the spline-based methods exhibit noticeably larger local perturbations. Because splines impose less structural restriction than the parametric GSW framework, they are able to capture small fluctuations in the data, which results in more oscillatory forward rate behavior even when the general term structure remains similar.}
